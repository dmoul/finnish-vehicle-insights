...
This is because `vehicle_age` encompasses failures due to age as well as a lot of the information about distance driven. See @sec-causal-graph Causal graph and @tbl-model-year-distance-correlations High correlation among `model_year`, `median_km_driven` and `average_km_driven`.

### Separate linear regressions for each brand

For each brand separately, I calculated *lm(failure_rate ~ vehicle_age)* and *lm(failure_rate ~ median_km_driven_10K)*. I excluded brands that did not have at least `r brands_cutoff_modeling_min` data points.

```{r}
#| label: define-lm-brand-vehicle_age

my_lm_model_year <- function(x) {
  lm(failure_rate ~ vehicle_age,
           data = x)
}
...
mod1_set <- data_for_model %>%
  ...
  nest(data = c(failure_rate, vehicle_age),
       .by = brand) |>
  mutate(mod1 = map(data, my_lm_model_year),
...
Given the wide range of the 95% confidence intervals, a relative difference in brand quality can be determined in the regression with `vehicle_age` only for the highest and lowest quality brands (where the intervals do not overlap in @fig-relative-brand-quality-lm). The narrower confidence intervals in the regression using `median_km_driven_10K` provide a larger set of relative higher-quality and lower-quality brands.
...
p1 <- mod1_set_summary |>
  ...
  geom_point(aes(x = estimate, y = term),
             ...
             show.legend = FALSE) +
  labs(
    subtitle = "Increment: year of vehicle_age",
    y = NULL,
    tag = "A"
  )
...
mod1_set_summary |>
  ...
  fmt_number(columns = c(estimate:plus_minus),
             decimals = 3) |>
  tab_source_note(md("*For each brand, calculated separately: lm(failure_rate ~ vehicle_age)*"))
...
mod2_set_summary |>
  ...
  fmt_number(columns = c(estimate:plus_minus),
             decimals = 3)|>
  tab_source_note(md("*For each brand, calculated separately: lm(failure_rate ~ median_km_driven_10K)*"))
